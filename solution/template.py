...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        for chunk in stream:
            ...
            reply += delta
        print()  # xuống dòng sau khi in xong 1 lượt trả lời
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    header = (
        f"{'Prompt':<42} | {'GPT-4o Response':<43} | {'Mini Response':<43} "
        f"| {'GPT-4o Latency':<15} | {'Mini Latency':<15}"
    )
    lines = [header, "-" * len(header)]

    for r in results:
        line = (
            f"{truncate(r['prompt']):<42} | "
            f"{truncate(r['gpt4o_response']):<43} | "
            f"{truncate(r['mini_response']):<43} | "
            f"{r['gpt4o_latency']:<15.3f} | "
            f"{r['mini_latency']:<15.3f}"
        )
        lines.append(line)
